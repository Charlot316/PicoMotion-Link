    ...
    while True:
        ...
        for key in ["head", "left", "right"]:
            ...
            if s["pos"] and s["initial_pos"]:
                dx, dy, dz = s["pos"]["x"]-s["initial_pos"]["x"], s["pos"]["y"]-s["initial_pos"]["y"], s["pos"]["z"]-s["initial_pos"]["z"]
                if key == "head":
                    ...
                    line = f"{key.upper():5} | Pos: {dx:>6.2f}, {dy:>6.2f}, {dz:>6.2f} | Ang: {s['euler'][0]:>4.0f}, {s['euler'][1]:>4.0f}, {s['euler'][2]:>4.0f} | [{t}{g}{b1}{b2}] | JS:({jx:>5.2f}, {jy:>5.2f})"
...
def receive():
    try:
        ...
        if dtype == "controller":
            for i, b in enumerate(data.get("buttons", [])): states[key]["btns"][i] = b['pressed']
            states[key]["axes"] = data.get("axes", [0,0])
            if states[key]["btns"].get(3):
                for k in states: 
                    if states[k]["pos"]: states[k]["initial_pos"] = states[k]["pos"]
        if states[key]["initial_pos"] is None: states[key]["initial_pos"] = states[key]["pos"]
